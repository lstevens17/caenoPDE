...
def print_usage():
    print("Usage: python nigon_oxford.py <sp1_busco_table_file> <sp2_busco_table_file> <nigon_table_file>")
    print("Arguments:")
    print("  <sp1_table_file>    Path to the first species table file")
    print("  <sp2_table_file>    Path to the second species table file")
    print("  <nigon_table_file>  Path to the nigon mapping table file")
...
def validate_arguments():
    ...
    for i, arg in enumerate(sys.argv[1:], start=1):
        if not os.path.isfile(arg):
            print(f"Error: The file specified in argument {i} ('{arg}') does not exist.")
            print_usage()
            sys.exit(1)
...
def print_comparison(sp1_table_dict, sp2_table_dict, busco2nigon_dict):
    for buscoID, sp1_pos_info in sp1_table_dict.items():
        if buscoID in sp2_table_dict:
            ...
            nigon = busco2nigon_dict.get(buscoID, "unassigned")
            print(buscoID + "\t" + nigon + "\t" + "\t".join(sp1_pos_info) + "\t" + "\t".join(sp2_pos_info))

if __name__ == "__main__":
    validate_arguments()  # Check arguments before proceeding
